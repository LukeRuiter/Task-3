...
namespace Task1_POE//Thee0
{
    public partial class Form1 : Form
    {
        ...
        private void updatebuttons()
        {
            ...
            for (int x = 0; x< 20; x++)
            {
                for (int y = 0; y< 20;y++)
                {
                    for (int i =0; i< 2; i++)
                    {
                        if (x == GameMap.WizardList[i].X && y == GameMap.WizardList[i].Y)
                        {
                            buttonarray[x, y].Text = GameMap.WizardList[i].Symbol.ToString();

                            switch (GameMap.WizardList[i].team)
                            {
                                case "Blue":
        ...
        public void button_Click(object sender, EventArgs e)
        {
            ...
            if (((MyButton)sender).Text == "S" || ((MyButton)sender).Text == "A" || ((MyButton)sender).Text == "R"|| ((MyButton)sender).Text == "F"|| ((MyButton)sender).Text=="W")
            {
                for (int i = 0; i < 2; i++)
                {
                    Wizard w = (Wizard)GameMap.WizardList[i];

                        if (((MyButton)sender).x == w.X && ((MyButton)sender).y ==w.Y)
                    {
